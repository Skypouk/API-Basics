...
@user_router.post("/users/", response_model=users.User)
...
@user_router.get("/users/", response_model=List[users.User])
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = UserCRUD.get_users(db, skip=skip, limit=limit)
    return users


@user_router.get("/users/{user_id}", response_model=users.User)
